...
type Config struct {
	...
	InstanceIdleTimeoutMinutes int
...
func Load() *Config {
	...
	instanceIdleTimeout := os.Getenv(config_env.INSTANCE_IDLE_TIMEOUT)
	...
	config := &Config{
		...
		InstanceIdleTimeoutMinutes: parseIntOrDefault(instanceIdleTimeout, 0),
